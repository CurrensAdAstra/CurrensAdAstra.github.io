---
title: "Curriculum Vitae"
category: "personal"
lang: ko
translation_key: cv
---

# Min-Gi Kyung, Ph.D.

임베디드 시스템 & 로보틱스 연구자  
서울, 대한민국

Email: MinGi.KYUNG@gmail.com  
ORCID: 0000-0001-8071-8511

---

## Research Profile

실시간 플랫폼, 로보틱스 미들웨어, 성능 중심 시스템 설계를 전문으로 하는 임베디드 시스템 연구자입니다.

Linux 기반 임베디드 플랫폼, UAV 시스템, GPU 가속 시뮬레이션을 다루며 학술 모델링과 프로덕션급 소프트웨어 시스템을 연결하는 연구를 수행합니다.

---

## Research Interests

- 임베디드 시스템 및 실시간 플랫폼
- 로보틱스 시스템 및 ROS 2 미들웨어
- GPU 가속 시뮬레이션 및 성능 모델링
- 하드웨어–소프트웨어 코디자인

---

## Education

### Ph.D. in Computer & Information Communication Engineering
**Konkuk University**, Seoul, Republic of Korea  
2010 – 2019

**Dissertation:** GPU-based Parallel LCTM Traffic Simulation Time Prediction

### M.S. in Computer & Information Communication Engineering
**Konkuk University**, Seoul, Republic of Korea  
2008 – 2010

**Thesis:** Conformance Testing Framework for Automotive Software

### B.S. in Computer Engineering
**Konkuk University**, Seoul, Republic of Korea  
2001 – 2008

---

## Research Experience

### Postdoctoral Researcher
**Konkuk University**  
Mar 2019 – Dec 2019

- GPU 가속 교통 시뮬레이션 연구
- CUDA 병렬 모델 개발
- CPU 대비 성능 검증
- 학술 출판/연구 과제 지원

---

## Industry Experience

### Embedded Linux Systems Engineer
**Kaon Group**  
Nov 2023 – Mar 2025

- ARM SoC 기반 Linux 임베디드 플랫폼 개발/유지보수
- 커널 드라이버 및 브링업 코드 구현
- Dolby MS12 v1.3.4, TB-44 인증 대응
- 장기 운영 신뢰성 중심 유지보수

### Senior Research Engineer (Robotics Systems)
**Argosdyne Co., Ltd.**  
Jan 2023 – Oct 2023

- ROS 2 기반 UAV 제어 아키텍처 설계
- ArduPilot + ROS 2 통합
- 사설 5G 기반 실시간 영상 전송 시스템 개발
- STM32 펌웨어 구현

### Research Engineer (GIS Systems)
**Wise Infotech Co., Ltd.**  
Jul 2021 – Nov 2022

- GIS 기반 인프라 관리 시스템 개발
- Spring Framework + Leaflet.js 기반 웹 시스템 구축

---

## Teaching Experience

### Lecturer
**Nazareth University** (2013)
- Operating Systems
- Software Development Project

**Hansung University** (2013)
- Data Communications

**Kookmin University** (2015)
- Object-Oriented Programming

### Teaching Assistant / Research Assistant
**Konkuk University**  
Mar 2008 – Feb 2019

- 소프트웨어공학/임베디드 과목 실습 지원
- 프로젝트 기반 수업 운영 보조

---

## Publications

전체 논문 목록은 [Publications](/contents/personal/publications-ko/)에서 확인할 수 있습니다.

---

## Research Projects

**Cloud-Based Traffic Simulation Project (ETRI Collaboration)**  
2017 – 2019

- GPU 가속 교통 시뮬레이션 모델 개발
- 대규모 도시 교통 성능 분석

---

## Patents

**Method for Setting Flight Path and Aircraft**  
2012 (Registered, Expired)

---

## Technical Skills

**Programming**  
C, Embedded C, Rust, Java

**Operating Systems**  
Linux, Embedded Linux, QNX

**Robotics Systems**  
ROS 2, ArduPilot

**Parallel Computing**  
CUDA, OpenMPI

**Embedded Platforms**  
ARM SoCs, STM32

**Web / GIS Systems**  
Spring Framework, Leaflet.js

---

## Certifications

- Engineer Information Processing
- Industrial Engineer Information Processing
- Craftsman Information Processing

---

## Academic Service & Outreach

- 운영체제/네트워킹/객체지향/소프트웨어공학 강의
- 대학/지역사회 기반 기술 교육 프로그램 참여

---

_Last updated: {{ "now" | date: "%Y-%m-%d" }}_
